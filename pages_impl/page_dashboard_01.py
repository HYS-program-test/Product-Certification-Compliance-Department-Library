...
LEGEND_TOP = dict(orientation="h", y=1.18, x=0, font=dict(size=11))
...
def render():
    ...
    with col1, st.container(key="chart_card_01_1"):
        ...
        fig.update_layout(barmode="group", height=260, margin=MARGIN, legend=LEGEND_TOP,
    ...
    with col4, st.container(key="chart_card_01_4"):
        ...
        fig.update_layout(barmode="stack", height=280, margin=MARGIN, legend=LEGEND_TOP,
                           plot_bgcolor="white", paper_bgcolor="white")
        st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": False})
    ...
    with col5, st.container(key="chart_card_01_5"):
        ...
        # 由左到右：<100% → 100-102.9% → 103-105.9% → ≥106%（跟截圖疊圖方向一致）
        order = ["<100%", "100-102.9%", "103-105.9%", "≥106%"]
        colors = {"<100%": "#C0504D", "100-102.9%": "#4472C4", "103-105.9%": "#82BE7E", "≥106%": "#8064A2"}
        fig = go.Figure()
        for bucket in order:
        ...
        fig.update_layout(barmode="stack", height=280, margin=MARGIN,
                           legend=dict(orientation="h", y=1.18, x=0, font=dict(size=11), traceorder="reversed"),
                           plot_bgcolor="white", paper_bgcolor="white")
        st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": False})
